...
- [`maptiles`](https://github.com/riatelab/maptiles) 
    ...
    - downloads it as a raster, so Rachel uses tidyterra to do something with it then...
...
## Rachel is not sure that reprojecting your base map is a good idea....compare the coloring of the two maps produced.
## She recommends reprojecting your data to your base map.
...
ca_osm <- get_tiles(ca3857, crop = TRUE)   # downloads base map layer
...
ggplot(data = ca_proj_sf) + # specify data here to set CRS 
  ...
  ggspatial::annotation_scale(
    pad_y = unit(0.5, "cm"), 
    width_hint = 0.3, # want it to take about 30% of the width of the plot
    unit_category = "imperial"  # change from metric to imperial units (km to miles)
    ) +
  ggspatial::annotation_north_arrow(
    pad_y = unit(1, "cm"),
    pad_x = unit(.75, "cm"),
